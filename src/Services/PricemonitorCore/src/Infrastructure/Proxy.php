...
class Proxy extends BaseProxy
{
    ...
    {
        $callbacksForRequest['pricemonitorCompleted'] = array();

        /** @var CallbackDTO $callbackDTO */
        foreach ($callbackDTOs as $callbackDTO) {
            $bodyTemplate = '';
            $callbackBodyTemplateDecoded = $callbackDTO->getBodyTemplate();
            if (!empty($callbackBodyTemplateDecoded)) {
                $bodyTemplate = json_encode($callbackBodyTemplateDecoded);
            }
            ...
            $callbacksForRequest['pricemonitorCompleted'][] = array(
                ...
                'headers' => !empty($headers) ? $headers : array('Cache-Control' => 'no-cache'),
