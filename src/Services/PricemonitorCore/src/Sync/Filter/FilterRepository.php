...
class FilterRepository
{
    ...
    public function saveFilter($contractId, Filter $filter)
    {
        ...
        $childrenErrors = $error->getChildrenErrors();
        $ownErrors = $error->getOwnErrors();

        if (!empty($childrenErrors) || !empty($ownErrors)) {
    ...
    public function getFilter($contractId, $type)
    {
        ...
        $transactionHistoryValidator->validateType($type);
